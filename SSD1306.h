...
#define TFTSECS 5
scrseg_struct     tftdata[TFTSECS] =                                // Screen divided in 5 segments + 1 overlay
{                                                                   // One text line is 8 pixels
  { false, YELLOW,  0,  8, "" },                  //0               // cursor 0 ===1 lines on top 
  { false, YELLOW,  8, 16, "" },                  //1
  { false, YELLOW, 16, 48, "" },                  //2               // 4 lines in the middle
  { false, YELLOW, 56, 64, "" },                  //3               // 3 lines at the bottom, leave room for indicator
  { false, GREEN,  40, 22, "" }                   //4               // 3 lines at the bottom for rotary encoder
} ;
...
#define OLED_CMD_SET_COM_SCAN_MODE      0xC8    // 0xC8 to flip
...
//#define OLED_CMD_SET_FLIP_SCREEN        0xc8
...
void SSD1306::display()
{
  ...
  for ( pg = 0 ; pg < SSD1306_NPAG ; pg++ )
  {
    if ( ssdbuf[pg].dirty )
    {
      ...
      //i2c_master_write_byte ( cmd, OLED_CMD_SET_COM_SCAN_MODE, true ) ;   //flip the screen
      
  // i2c_master_write_byte(cmd, 0x12,true);    // for 0.91 size
  // i2c_master_write_byte(cmd, 3, true); // Page end address
      i2c_master_stop ( cmd ) ;
...
SSD1306::SSD1306 ( uint8_t sda, uint8_t scl )
{
  ...
  i2c_master_write_byte ( cmd, OLED_CMD_SET_COM_SCAN_MODE, true ) ;   //flip the screen
  i2c_master_write_byte ( cmd, OLED_CMD_SET_SEGMENT_REMAP, true ) ;//rotation
   // i2c_master_write_byte(cmd, 3, true); // Page end address
  i2c_master_stop ( cmd ) ;
